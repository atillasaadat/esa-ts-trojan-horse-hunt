...
def visualise_trigger(
    trig_np: np.ndarray, model_id: int, out_dir: Path, show: bool = False
):
    """
    ...
    for c in range(C):
        axes[1].stem(
            freqs, fft[:, c], linefmt=f"C{c}-", markerfmt=f"C{c}o", basefmt=" "
        )
...
if __name__ == "__main__":
    # Example usage
    trig_np = np.random.randn(75, 3)  # Dummy trigger data
    model_id = 17
    out_dir = Path("output")
    visualise_trigger(trig_np, model_id, out_dir, show=True)
